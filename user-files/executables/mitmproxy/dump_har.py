...
def load(l):
    ...
    l.add_option(
        "jsondump", str, "", "JSON dump path.",
    )
    l.add_option(
    	        "request_host", str, "", "Filter request host"
    	    )
    l.add_option(
    	        "request_path", str, "", "Filter request path"
    	    )
...
def response(flow):
    ...
    # Filter the request host and path
        request_hosts = ctx.options.request_host
        request_hosts_array = request_hosts.split(",")
        request_hosts_array.append(ctx.options.request_path)
        filter_string = (i for i in request_hosts_array if i)
        flag = False
        for i in filter_string:
            print("Filter String :: " + i)
    ...
    HAR_JSON["log"]["entries"].append(entry)
    
    if ctx.options.hardump:
        json_dump: str = json.dumps(HAR, indent=2)
        dump: str = json.dumps(HAR_JSON, indent=2)
    
        if ctx.options.hardump == '-':
            mitmproxy.ctx.log(json_dump)
        else:
            raw: bytes = json_dump.encode()
            raw_dump: bytes = dump.encode()
            if ctx.options.hardump.endswith('.zhar'):
                raw = zlib.compress(raw, 9)
            
            with open(os.path.expanduser(ctx.options.hardump), "wb") as f:
                f.write(raw)
            with open(os.path.expanduser(ctx.options.jsondump), "wb") as f:
                f.write(raw_dump)

    if len(HAR_JSON["log"]["entries"]) >= 10:
        HAR_JSON.update({
                 "log": {
                 "version": "1.2",
                 "creator": {
